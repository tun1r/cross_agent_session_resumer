...
fn read_grok_fixture() -> CanonicalSession {
    ...
    Grok.read_session(&path)
        .unwrap_or_else(|e| panic!("Failed to read Grok fixture: {e}"))
}
...
fn roundtrip_cc_to_grok() {
    ...
    let written = Grok
        .write_session(&original, &WriteOptions { force: false })
        .expect("CC→Grok: write should succeed");

    let readback = Grok
        .read_session(&written.paths[0])
        .expect("CC→Grok: read-back should succeed");
...
fn roundtrip_grok_to_cc() {
    ...
    let written = ClaudeCode
        .write_session(&original, &WriteOptions { force: false })
        .expect("Grok→CC: write should succeed");

    let readback = ClaudeCode
        .read_session(&written.paths[0])
        .expect("Grok→CC: read-back should succeed");
